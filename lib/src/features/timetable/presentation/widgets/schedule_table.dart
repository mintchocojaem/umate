...
class ScheduleTable extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    List<Widget> addAllScheduleInfo(
      ...
      final tableStartDuration = ScheduleTimeFormat(
        hour: tableStartHour,
        minute: 0,
      );
      ...
      for (Schedule i in schedules) {
        for (ScheduleTime j in i.times) {
          final scheduleHeight =
              (j.endTime.subtract(j.startTime).hour * scheduleWidgetHeight) +
                  (j.endTime.subtract(j.startTime).minute *
                      scheduleWidgetHeight /
                      60);

          final scheduleTextMaxLine =
              (((j.endTime.subtract(j.startTime).hour * 60) +
                      j.endTime.subtract(j.startTime).minute) ~/
                  30);

          scheduleWidgets.add(
            Positioned(
              top: (j.startTime.subtract(tableStartDuration).hour *
                      scheduleWidgetHeight) +
                  (j.startTime.subtract(tableStartDuration).minute *
                      scheduleWidgetHeight /
                      60),
